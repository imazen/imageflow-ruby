module Imageflow
  class JobContext
    ...
    def build (build: )
      self.send_json(method:"v1/build", data: build, raise_error: true)
    end

    def execute (framewise: )
      self.send_json(method: "v1/execute", data: {"framewise": framewise}, raise_error: true)
    end

    def get_image_info(io_id:)
      result = self.send_json(method: "v1/get_image_info", data:{"io_id": io_id}, raise_error: true)
    ...
    def set_decoder_downscale_hints(io_id:, downscaled_min_width:,
      ...
      result = send_json("v1/tell_decoder", {"io_id": io_id, "command": {"jpeg_downscale_hints": hints}}, raise_error: true)
      raise result.message unless result.ok?
    end
